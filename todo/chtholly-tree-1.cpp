...
public:
    ...
    value_type rank(size_type __l, size_type __r, size_type __k)
    {
        ...
        for (typename std::vector<_Pair_t>::iterator __it = __tmp.begin();
            __it != __tmp.end(); ++__it)
        {
            __cnt += __it->second;
...
int main()
{
    ...
    std::cin >> n >> m >> seed >> rmod;

    for (size_t i = 0; i < n; ++i)
        data.push_back(rnd() % rmod + 1);
